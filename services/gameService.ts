...
import { MY_LIST, MY_INFO, QUESTION_BANKS } from '../data/mockData'; 
...
function transformQuestion(rawData: any): Question {
 ...
 */
export async function getAllQuestions(topics: Topic[]): Promise<Question[]> {
    try {
 ...
 */
export async function submitAnswer(topicId: string, questionId: string, selectedOption: string): Promise<void> {
    const params = new URLSearchParams({
        action: 'submit',
        topicId: topicId,
...
export async function getSpreadsheetName(): Promise<{ success: boolean; name?: string; message: string; }> {
...
export async function testWriteConnection(): Promise<{ success: boolean; message: string }> {
...
export async function testTopicsConnection(): Promise<{ success: boolean; message: string; topics?: Topic[] }> {
    const params = new URLSearchParams({ action: 'getTopics' });
    params.set('v', new Date().getTime().toString()); // Cache buster
    const url = `${GOOGLE_SHEET_URL}?${params.toString()}`;
...
export async function testReadConnection(topicId: string): Promise<{ success: boolean; message: string }> {
    const params = new URLSearchParams({
        action: 'getQuestions',
        topicId: topicId
    });
