...
import { testWriteConnection, testReadConnection, testTopicsConnection, getSpreadsheetName } from '../services/gameService';
import Card from './ui/Card';
import Button from './ui/Button';

// Định nghĩa các trạng thái có thể có cho mỗi bài kiểm tra
type TestStatus = 'idle' | 'testing' | 'success' | 'error' | 'skipped';

// Cấu trúc để lưu trữ kết quả của một bài kiểm tra
...
const StatusIcon = ({ status }: { status: TestStatus }) => {
    switch (status) {
        ...
        default:
            return <div className="w-4 h-4 bg-gray-500 rounded-full opacity-50"></div>;
    }
};

const APITestCard: React.FC = () => {
    const [isTesting, setIsTesting] = useState(false);
    const [connectionResult, setConnectionResult] = useState<TestResult>({ status: 'idle', message: 'Kiểm tra tệp Google Sheet được kết nối.' });
    const [writeResult, setWriteResult] = useState<TestResult>({ status: 'idle', message: 'Kiểm tra khả năng GHI dữ liệu vào Sheet.' });
    const [topicResult, setTopicResult] = useState<TestResult>({ status: 'idle', message: 'Kiểm tra khả năng TẢI danh sách chủ đề.' });
    const [readResult, setReadResult] = useState<TestResult>({ status: 'idle', message: 'Kiểm tra khả năng ĐỌC câu hỏi từ Sheet.' });
    ...
    const TestItem = ({ title, result }: { title: string; result: TestResult }) => (
